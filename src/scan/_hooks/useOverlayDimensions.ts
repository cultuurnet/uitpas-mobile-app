...
export function useOverlayDimensions(dimensions, { padding, strokeWidth, cornerLength }: TOverlayDimensions) {
  ...
  return useMemo(() => {
    ...
    const regionDefinition = {
      Bottom: Math.round(((boundingBox.bottom / height) * 100) / 5) * 5,
      Left: Math.round(((boundingBox.left / width) * 100) / 5) * 5,
      Right: Math.round(((boundingBox.right / width) * 100) / 5) * 5,
      Top: Math.round(((boundingBox.top / height) * 100) / 5) * 5,
    };
